...
<?php $sql = "SELECT editortable.id,editortable.paperid,editortable.username,editortable.primaryemail,editortable.assigndate,editortable.assignmonth,editortable.assignyear,editortable.action from editortable where feedback IS NOT NULL and primaryemail='$useremails'";
$query = $dbh->prepare($sql); 
$query->execute(); 
$results=$query->fetchAll(PDO::FETCH_OBJ); 
$cnt=1;
...
<?php 
  ...
  echo "<script>alert('You are not logged in as a Editor.Try to log in as an Editor');</script>";
